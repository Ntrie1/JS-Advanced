...
async function getInfo() {
    ...
    const inputFieldEl = document.getElementById('stopId').value;
    const divElement = document.getElementById('stopName');
    const ulElement = document.getElementById('buses');
    const url = `http://localhost:3030/jsonstore/bus/businfo/${inputFieldEl}`;
    ...
    try {
        ...
        Object.keys(data.buses).forEach((bus) => {
            ...
            liEl.textContent = `Bus ${bus} arrives in ${bus[data.buses]} minutes`;
